...
int osUtf8Encode(osPrintf_t* pf, const char *str)
{
	...
	while (*str) 
	{
		...
		if (unicode)
		{
			...
			err = pf->pHandler(ubuf, sizeof(ubuf), pf->arg);
		}
		else if (ec) 
		{
			ebuf[1] = ec;

			err = pf->pHandler(ebuf, sizeof(ebuf), pf->arg);
		}
		else 
		{
			err = pf->pHandler((char *)&c, 1, pf->arg);
		}
		...
		if (ch == '\\')
		{
			...
			switch (ch)
			{
				...
				case 'u':
					...
					if (!isxdigit(pl->p[i+1]) || !isxdigit(pl->p[i+2]) || !isxdigit(pl->p[i+3]) || !isxdigit(pl->p[i+4]))
					{
						return EBADMSG;
					}

					u |= ((uint16_t)osStr_chHex2Bin(pl->p[++i])) << 12;
					u |= ((uint16_t)osStr_chHex2Bin(pl->p[++i])) << 8;
					u |= ((uint16_t)osStr_chHex2Bin(pl->p[++i])) << 4;
					u |= ((uint16_t)osStr_chHex2Bin(pl->p[++i])) << 0;
					...
					ulen = osUtf8EncodeByteseq(ubuf, u);

					err = pf->pHandler(ubuf, ulen, pf->arg);
					if (err)
					{
						return err;
					}
