...
#if 0
void osPreMem_usedInfo(uint8_t idx)
{
    if(idx >= OS_PREMEM_MAX_IDX)
    {
...
#else
    int count = 0;
	osPreMemBlockHdr_t** unusedList = malloc(sizeof(osPreMemBlockHdr_t*)*osPreMemNum[idx]);
    ...
    while(pBlock != NULL)
    {
        unusedList[count++] = pBlock;
        pBlock = pBlock->nextBlock;
    }

    pthread_mutex_unlock(&osPreMemUnused[idx].mutex);

	osPreMemBlockHdr_t** totalList = malloc(sizeof(osPreMemBlockHdr_t*)*osPreMemNum[idx]);
	for(int i=0; i<osPreMemNum[idx]; i++)
	{
		totalList[i] = ((void*)osPreMemUnused[idx].blockStart) + (sizeof(osPreMemBlockHdr_t)+osPreMemSize[idx])*i;
	}

	for(int i=0; i<count; i++)
	{
		for(int j=0; j<osPreMemNum[idx]; j++)
		{
			if(unusedList[i] == totalList[j])
			{
				totalList[j] = 0;
				break;
			}
	...
	int n=0;
	char* dbgPrint = malloc(count*OS_PREMEM_MAX_DEBUG_SIZE);
	for(int i=0; i<osPreMemNum[idx]; i++)
	{
		if(totalList[i] != 0)
		{
			n += sprintf(&dbgPrint[n], "%s", totalList[i]->dbgInfo);

//			logInfo("used prememory (size=%d): %s", osPreMemSize(idx), ((osPreMemBlockHdr_t*)totalList[i])->dbgInfo);
		}
	}	

    logInfo("used prememory (size=%d):\n%s", osPreMemSize[idx], dbgPrint);
